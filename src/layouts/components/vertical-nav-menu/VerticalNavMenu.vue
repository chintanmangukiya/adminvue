...
<v-navigation-drawer :value="isDrawerOpen" app floating width="260" class="app-navigation-menu" :right="$vuetify.rtl" @input="val => $emit('update:is-drawer-open', val)">
    ...
    <v-list expand shaped class="vertical-nav-menu-items pr-5">
        ...
        <nav-menu-link title="User" :to="{ name: 'user' }" :icon="icons.mdiAccount"></nav-menu-link>
    ...
    <!-- <a href="https://themeselection.com/products/materio-vuetify-vuejs-admin-template" target="_blank" rel="nofollow">
        <v-img :src="require(`@/assets/images/pro/upgrade-banner-${$vuetify.theme.dark ? 'dark' : 'light'}.png`)" alt="upgrade-banner" transition="scale-transition" class="upgrade-banner mx-auto" max-width="230"></v-img>
    </a> -->
</v-navigation-drawer>
</template>
